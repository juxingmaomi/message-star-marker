...
- 点击 TavernHelper 或右下角唯一的 `楼层书签跳转` 按钮时打开标记列表。
- 列表支持按全部 / 问答 / 来信 / 星标 / 爱心筛选，点击楼层可跳转，也可直接取消对应标记。
- 如果楼层正文里有 `<Scene_Title>标题</Scene_Title>`，列表会显示这个标题。
- 点击未加载的旧楼层时，会跳过酒馆原生 `/chat-jump` 的批量历史加载，直接分帧临时显示目标楼层前后各 3 楼，并可在面板里恢复完整聊天。
- 轻量区间模式提供“前一段 / 后一段”导航，可每次最多 7 楼地连续向前或向后阅读。
- 手机端会在页面恢复或重新可见时自动补回 `楼层书签跳转` 面板入口。
- 手机端不再运行时替换脚本按钮配置，并提供页面点击兜底，避免脚本 iframe 被反复重建。
...
```js
https://cdn.jsdelivr.net/gh/YOUR_GITHUB_USERNAME/message-star-marker@v0.5.6/index.js
```
